...
def contain_particle_chunk(chunk):
    particle_chunk = ''
    for morph in chunk.morphs:
        if morph.base != '*':
            particle_chunk += morph.base
            ...
            if judge(chunk,'動詞'):
                ...
                for kakarimoto_index in chunk.srcs: #動詞が見つかり文節の係り元リストの要素数だけループ
                    if judge(sentence[kakarimoto_index],'助詞'):
                        ...
                        kakarimoto_list.append(contain_particle_chunk(sentence[kakarimoto_index]))
                if len(particle_list) > 0:
                    verb = search_verb(chunk)
                    verb_particle_pair.append([verb,' '.join(particle_list),' '.join(kakarimoto_list)])
