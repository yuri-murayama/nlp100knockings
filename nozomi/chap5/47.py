        ...
        for cm in chunk.morphs:
            ...
            if cm.pos == "動詞":
                ...
                for sc in chunk.srcs:
                    num=-1
                    last_morph=koneko[sc].morphs[num]
                #for morph in koneko[sc].morphs:
                    if last_morph.surface=='、':
                        num-=1
                        last_morph=koneko[sc].morphs[num]
                    if ((last_morph.pos == '助詞' and last_morph.surface == 'を') and koneko[sc].morphs[num-1].pos1=='サ変接続'):
                        verb.append(koneko[sc].morphs[num-1].surface+'を'+cm.base)
                        #print(verb)
                    elif last_morph.pos=='助詞':
                        z_lst.append(last_morph.base)
                        zw_lst.append(''.join([m.surface for m in koneko[sc].morphs]))
                if verb != [] and z_lst!=[]:
                    print("{}\t{}\t{}\n".format(''.join(verb),''.join(z_lst),''.join(zw_lst),end="\t"))
